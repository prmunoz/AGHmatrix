...
This section presents how to load the data and how to construct the genomic-based relationship matrix for autopolyploid species. In the package, the function Gmatrix is the one that handles the molecular-markers matrix and builds the relationship matrix. Molecular markers data should be organized in a matrix format (individual in rows and markers in columns) coded according to the dosage level: 0,1,2,...,ploidy level, and missing data value (numeric or NA). As an example, an autohexaploid should be coded as 0,1,2,3,4,5,6, and missing data value. Import your molecular marker data into R with the function read.table() and convert to a matrix format with the function as.matrix(). In autopolyploids, the function Gmatrix can be used then to construct: i) the additive relationship based on VanRaden (2008) and also implemented at the `sommer` package (Covarrubias-Pazaran 2016); ii) the full-autopolyploid including non-additive model as Slater et al. (2016); iii) the pseudo-diploid model as Slater et al. (2016). As an example, here we build the three matrices using simulated data.
...
#Computing the full-autopolyploid matrix based on Slater 2016
G_Slater <- Gmatrix(markersdata, method="Slater", ploidy=4)

#Computing the pseudodiploid matrix based on Slater 2016
