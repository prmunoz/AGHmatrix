...
Loading the built-in molecular data derived from an autotetraploid potato population from Endelman (2018):
```{r}
data(snp.potato)
```
...
G_VanRaden <- Gmatrix(snp.potato, method="VanRaden", ploidy=4)

#Computing the dominance (digenic) matrix based on Endelman 2018 (Eq. 19)
G_Digenic <- Gmatrix(snp.potato, method="Endelman", ploidy=4)

#Computing the full-autopolyploid matrix based on Slater 2016 (Eq. 8 and 9)
G_FullAutopolyploid <- Gmatrix(snp.potato, method="Slater", ploidy=4)

#Computing the pseudodiploid matrix based on Slater 2016 (Eq. 5, 6, and 7)
G_Pseudodiploid <- Gmatrix(snp.potato, method="VanRaden", ploidy=4, pseudo.diploid=TRUE) 
```
...
## Combined relationship matrix - H matrix with real data
Here we present how to compute the H matrix using the package. This matrix is computed based on the matrix A using matrix G as a correction and it is presented in Munoz et al. 2014. In this example we consider the A matrix as an identity (*e.g* we do not have pedigree information and assume the individuals in Hardy-Weinberg equilibrium). It is important to pass to `Hmatrix` the same arguments of `missingValue` and `maf` of `Gmatrix`. If the `Hmat` gives singularity problem, you can try to add a small constant to its calculus changing argument `c` to a small value (*e.g*, `0.001`).

Here we are going to use the data from Endelman (2018) already built-in this present package.
...
## Computing the realized additive relationship matrix
Gmat <- Gmatrix(SNPmatrix = snp.potato, ploidy = 4, maf = 0.05, method = "VanRaden")

## Computing the expected relationship matrix
Amat <- Amatrix(data = ped.potato, ploidy = 4)

## Computing H matrix (it may take a few minutes)
Hmat <- Hmatrix(A=Amat, G=Gmat, markers=snp.potato, ploidy=4, maf=0.05)
```
