...
datatreat <- function(data=NULL,
    ...
    if( save ){
        cat( paste("Running pedigree reordering algorithm... \n",sep=""))
    }
    ...
    for( i in 1:n.max){
        ...
        ## Testing to avoid double replacement
        right.pos[which(duplicated(right.pos,incomparables=c(NA)))] <- NA
...
#            cat( paste(error,"\n"))
        ...
        ## Removing individuals duplicated
        ind[which(duplicated(ind))] <- which(is.na(match(1:length(ind),ind)))
        new.data <- data[ind,]
