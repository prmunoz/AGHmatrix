...
# Contains: plot_genealogy, ancestral, cartesian, cartesian.plot, cartesian.big.plot 
# 									
...
plot_genealogy <- function(data=data,
                           ind=NULL,
                           gen=20,
                           Amatrix=NULL,
                           fontsize=3,
                           angle.names=0,
                           big=FALSE){
  xaxis <- yaxis <- NULL ## def variables for later use (it avoids note on R CMD CHECK)
  ...
  if(!is.null(Amatrix)){
    data.plot$inbreed <- round(diag(Amatrix)[data.plot$data.anc]-1,4)
    inbreed <- TRUE
  }else(inbreed <- FALSE)
